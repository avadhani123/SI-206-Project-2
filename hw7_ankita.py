...
def get_user_tweets(data):
	f_tweets = []
	if data in CACHE_DICTION:
		t_results = CACHE_DICTION[data]
	else:
		t_results = api.user_timeline(data)
		CACHE_DICTION[data] = t_results
	t_cache = open(CACHE_FNAME, "w")
	t_cache.write(json.dumps(CACHE_DICTION))
	t_cache.close()
	for a in t_results:
		f_tweets.append(a)
	return f_tweets
...
tablespec = 'CREATE TABLE IF NOT EXISTS '
tablespec += 'Tweets (tweet_id PRIMARY KEY, '
tablespec += 'author TEXT, time_posted TIMESTAMP, tweet_text TEXT, retweets INTEGER)'
cursor.execute(tablespec)
...
for tweets in umsi_tweets:
	tweet_tup = (tweets['id'], tweets['user']['screen_name'], tweets['created_at'], tweets['text'], tweets['retweet_count'])
	cursor.execute(statement, tweet_tup)
...
t_query = "SELECT time_posted FROM Tweets"
cursor.execute(t_query)
tweet_posted_times = cursor.fetchall()
...
t_query = "SELECT * FROM Tweets WHERE retweets > 2"
cursor.execute(t_query)
...
t_query = "SELECT tweet_text from Tweets WHERE instr(tweet_text, 'RT')"
cursor.execute(t_query)
...
def get_twitter_users(x):
	found_names = (re.findall(r'@([A-Za-z0-9_]+)',x))
	return {z for z in found_names}
