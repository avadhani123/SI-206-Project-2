...
cache_file_name = "206project2_caching.json"
...
def find_urls(words):
	...
	return URLS
#this is the regular expression that will return the urls.
...
def get_umsi_data():
	...
	# Checking to see if umsi_data is within the cached dictionry we have saved
	else:
		data = requests.get(base, headers={'User-Agent': 'SI_CLASS'})
		#Requestiing the dta from SI Class and saving onto htmldoc to use in beautiful soup.
		htmldoc = data.text
		...
		# This searches through the headers for the directory and returns the pages
		return(pages)
get_umsi_data()
...
#calls out get_umsi data and puts it into the pages_dict
for p in pages_dict:
...
def get_five_tweets(search):
	...
	#searches the id for twitter
	if tweet_identity in CACHE_DICTION:
		#if the id is within the cached dictionary then it sets this data equal to a content dict
		content = CACHE_DICTION[tweet_identity]
		
	else:
		data = api.search(q=search)
		#if the data is not cached this allows it to be looked up using the api search 
		content = data["statuses"]
		#this manually finds the status data through the parsing through the status data through content
		CACHE_DICTION[tweet_id] =content
		#indexes the id data from the cached data and names it to the variable content as well 
		twitter_f = open(CACHE_FNAME, "w")
		#opens up the twitter cached file name, and loads the data onto the cache file 
		twitter_f.write(json.dumps(CACHE_DICTION))
		twitter_f.close()
#this sets the empty list of five_phrases and appends the top five tweets to the empty list
	five_tweets_phrases = []
...
#this invokes the five tweets with the phrase University of Michigan in them
...
#iterates through the tweets list and finds the urls and appends them to the empty list.
